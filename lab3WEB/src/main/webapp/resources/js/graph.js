class GraphDrawer {
    ...
    setCurrentR(r) {
        this.currentR = r;
        this.updateCurrentScalePoints();
    }

    updateCurrentScalePoints() {
        if (this.currentR === null) {
            this.currentScalePoints = [...this.points];
        } else {
            this.currentScalePoints = this.points.filter(point =>
                Math.abs(point.r - this.currentR) < 0.0001
            );
        }
    }
    ...
    drawTargetShape(r) {
        ...
        ctx.fillStyle = 'rgba(244,66,208,0.7)';
    ...
    drawGraph(r = null) {
        ...
        this.points.forEach(point => {
            const isCurrentScale = this.currentScalePoints.some(currentPoint =>
                Math.abs(currentPoint.x - point.x) < 0.0001 &&
                Math.abs(currentPoint.y - point.y) < 0.0001 &&
                Math.abs(currentPoint.r - point.r) < 0.0001
            );

            this.drawPoint(point.x, point.y, point.isHit, point.r, isCurrentScale);
        });
    }

    drawPoint(x, y, isHit, r, isCurrentScale = true) {
        if (this.currentR === null) {
            return;
        }
        ...
        if (!isCurrentScale) {
            ctx.fillStyle = isHit ? '#888888' : '#cccccc';
        } else {
